...
const LeagueDetailsNewsSection = ({ leagueId }) => {
  ...
  const { leaguePageNews, leaguePageNewsLoading } = useSelector(
    (state) => state.news
  )
  ...
  useFocusEffect(
    React.useCallback(() => {
      return () => {
        dispatch(resetNews('leaguePageNews'))
  ...
  return (
    <Tabs.FlatList
      ...
      refreshControl={
        <RefreshControl
          refreshing={leaguePageNewsLoading}
          onRefresh={onRefresh}
...
export default React.memo(LeagueDetailsNewsSection)
